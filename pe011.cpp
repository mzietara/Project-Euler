...
int main() {
	int** gridArray = makeGridArray(grid);
	...
	for (int i=0; i < 20; i++) {
		for (int j=0; j < 20; j++) {
			if (i < 17) {
				...
				largestProduct = max(largestProduct, product); //right
				if (j < 17) {
					product = gridArray[i][j] * gridArray[i+1][j+1] * gridArray[i+2][j+2] * gridArray[i + 3][j+3];
					largestProduct = max(largestProduct, product);	//right down diagonal			
				}
			}	
			if (j < 17) {
				product = gridArray[i][j] * gridArray[i][j+1] * gridArray[i][j+2] * gridArray[i][j+3];
				largestProduct = max(largestProduct, product);	//down diagonal		
			}
			if (i >3 && j <17) {
				product = gridArray[i][j] * gridArray[i-1][j+1] * gridArray[i-2][j+2] * gridArray[i-3][j+3];
				largestProduct = max(largestProduct, product);	//down diagonal					
...
int** makeGridArray(string grid) {
	//convert the string grid to a 2d array
	int** result = new int*[20];

	for (int i=0; i < 20; i++) {
		result[i] = new int[20];
		for (int j=0; j < 20; j++) {
			result[i][j] = stoi(grid.substr(60*i + 3*j + 1, 2));
		}
	}
	return result;
}
